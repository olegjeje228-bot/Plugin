...
# ==== ТОКЕН ====
# Токен лежит в отдельном файле рядом с ботом.
TOKEN_FILE = "/home/discord_bot/token.txt"
try:
    with open(TOKEN_FILE, encoding="utf-8") as _tf:
        TOKEN = _tf.read().strip()
except FileNotFoundError:
    raise SystemExit(
        f"Не найден файл с токеном: {TOKEN_FILE}\n"
        f"Создай его:  echo 'ТВОЙ_ТОКЕН' > {TOKEN_FILE}"
    )

if not TOKEN:
    raise SystemExit(f"Файл {TOKEN_FILE} пустой - впиши туда токен бота.")
...
CALLADMIN_CHANNEL_ID = 1527126877213429761
...
TRAFFIC_CHANNEL_ID = 1530205230120046652  # 0 = слать в UPDATE_CHANNEL_ID
